...
with open('MatGenOutput.csv', mode='w') as MatGenTestCSV:
    ...
    MatGenTestCSVWrite.writerow(["Formula", "Metal Symbol", "M Number", "X Symbol", "X Number", "Metal Mass",
                                 "Metal Density", "Metal Interatomic Distance", "Metal Covalent Radius", "X Mass",
                                 "X Density", "X Interatomic Distance", "X Covalent Radius"]) #Create headers in CSV file
    for M_MatN2 in M_Mat: #Two for loops are nested together to iterate through each combination of M and X materials, could use simpler method.
        for X_MatN2 in X_Mat:
            ps = pt.elements.symbol #Declare ps as shorthand for longer function
            MatGenTestCSVWrite.writerows([((M_MatN2 + "1" + X_MatN2 + "1"), M_MatN2, "1", *X_MatN2, "1",
            ...
            #mass etc.
            MatGenTestCSVWrite.writerows([((M_MatN2 + "1" + X_MatN2 + "2"), M_MatN2, "1", *X_MatN2, "2",
                                           ps(M_MatN2).mass, ps(M_MatN2).density, ps(M_MatN2).interatomic_distance,
                                           ps(M_MatN2).covalent_radius, ps(X_MatN2).mass, ps(X_MatN2).density,
                                           ps(X_MatN2).interatomic_distance, ps(X_MatN2).covalent_radius)])
            MatGenTestCSVWrite.writerows([((M_MatN2 + "2" + X_MatN2 + "1"), M_MatN2, "2", *X_MatN2, "1",
                                           ps(M_MatN2).mass, ps(M_MatN2).density, ps(M_MatN2).interatomic_distance,
                                           ps(M_MatN2).covalent_radius, ps(X_MatN2).mass, ps(X_MatN2).density,
                                           ps(X_MatN2).interatomic_distance, ps(X_MatN2).covalent_radius)])
